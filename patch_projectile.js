...
const newUpdateProjectile = `private updateProjectile(ctx: SimRenderContext) {
    ...
    if (this.lastProjectileParamsStr !== currentParamsStr) {
      ...
      while(t < 25 && x < 85 && bounces < 3 && x > -10) {
        t += dt;
        x += vx * dt;
        y += vy * dt - 0.5 * g * dt * dt;
        vy -= g * dt;
        
        const groundY = Math.max(0, x) * Math.tan(alpha); // Assuming ground at x<0 is just horizontal, or plane extends?
        // Actually, let plane be straight line
    ...
    if (shellGroup) {
      ...
      // We can hide it shortly after max time
    ...
    if (targetGroup) {
      ...
      targetGroup.rotation.z = alpha; // Align with plane
    ...
    // Actually the current model is a shell, but it has dummy parts (ragdoll).
    ...
    if (head) {
      if (tInCycle >= T) {
        ...
        const flailFactor = Math.sin(tInCycle * 25) * Math.max(0, 1 - (tInCycle - T));
